...
def apply_all_rules_to_all_transactions():
    for transaction_id, transaction in st.session_state.transactions.items():
        st.session_state.transactions[transaction_id] = apply_rules_to_transaction(transaction)

def apply_rules_to_transaction(transaction):
    for rule in st.session_state.rules.values():
        keywords = [contains.strip() for contains in rule['contains'].lower().split(',')]
        if rule['field'] == 'beneficiary' and any(keyword in transaction['beneficiary'].lower() for keyword in keywords):
            transaction['category'] = rule['category']
        elif rule['field'] == 'description' and any(keyword in transaction['description'].lower() for keyword in keywords):
            transaction['category'] = rule['category']
        else:
