...
def account_exists(account: dict) -> bool:
    ...
    for existing_account in st.session_state.accounts.values():
        if account['name'] == existing_account['name'] or account['iban'] == existing_account['iban']:
            return True
    return False
...
def account_is_valid(account: dict) -> bool:
    ...
    if account_exists(account):
        st.error(f'Account with name "{account["name"]}" or IBAN "{account["iban"]}" already exists.')
        return False
    return True
...
def main():
    ...
    account_to_delete = st.selectbox('Account to delete', st.session_state.accounts.keys())
